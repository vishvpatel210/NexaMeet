import { IMeeting, ISearchResult } from '@shared/types/index';
...
export class ApiService {
   ...
   */
  static async getMeetings(params?: { category?: string; starred?: boolean; search?: string }): Promise<IMeeting[]> {
    try {
      const queryParts: string[] = [];
      if (params?.category && params.category !== 'All') queryParts.push(`category=${encodeURIComponent(params.category)}`);
      if (params?.starred) queryParts.push('starred=true');
   ...
   */
  static async createMeeting(payload: { title: string; category: string; location?: string }): Promise<IMeeting | null> {
    try {
   ...
   */
  static async searchSemantic(query: string, category?: string): Promise<ISearchResult[]> {
    try {
   ...
   */
  static async toggleStar(meetingId: string, currentStarred: boolean): Promise<IMeeting | null> {
    try {
